...
for model_key, label in model_DL.items():
    total_asset = np.mean(model_DL[model_key].sim.actions[:, :, 1].cpu().numpy()) + np.mean(model_DL[model_key].sim.actions[:, :, 2].cpu().numpy()) +np.mean(model_DL[model_key].sim.actions[:, :, 3].cpu().numpy())
...
axes[0,0].legend()
axes[0,0].set_title('Debt Share')
axes[0,1].set_title('Equity Share')
axes[1,0].set_title('Bond Share')
axes[1,1].set_title('House Share')


figs, axes = plt.subplots(3, 2, figsize=(16, 9))
...
for model_key, label in model_DL.items():
    ...
    axes[2,0].plot(np.mean(model_DL[model_key].sim.outcomes[:, :,  4].cpu().numpy(), axis=1), lw = 2) # debt share
    axes[2,1].plot(np.mean(model_DL[model_key].sim.outcomes[:, :,  5].cpu().numpy(), axis=1), lw = 2) # debt share
figs.suptitle("Outcomes patient beta = 0.99")

axes[0,0].legend()
axes[0,0].set_title('Consumption')
axes[0,1].set_title('House Holdings')
axes[1,0].set_title('Funds')
axes[1,1].set_title('Bonds')
axes[2,0].set_title('Equity')
axes[2,1].set_title('New Debt')
