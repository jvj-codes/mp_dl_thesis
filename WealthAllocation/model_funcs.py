...
def outcomes(model, states, actions, t0 = 0, t=None):
    ...
    if t is None: #when solving for DeepVPD or DeepFOC
        ...
        if pi.ndim == 3:  # (T, N, Nquad) → solving mode
            ...
            kappa = par.kappa[:T].reshape(-1,1).repeat(1,N) / (1 + pi)
   ...
   # print(f"income at time t {income}")
    ...
    #epsilon = 1e-6
    #c = torch.clamp(c, min=epsilon)
    #h_n = torch.clamp(h_n, min=epsilon)

    ## new debt 
    if t is None:
        T = w.shape[0]
    
        if w.ndim == 2:  # [T, N] → normal mode
            mask = (torch.arange(T, device=w.device) < par.T_retired).float().reshape(-1, 1)
        elif w.ndim == 3:  # [T, N, Nquad] → quadrature mode
            mask = (torch.arange(T, device=w.device) < par.T_retired).float().reshape(-1, 1, 1)
        else:
            raise RuntimeError("Unexpected shape for `w`")
    
        d_n = alpha_d * par.vartheta * w * n_act * mask
    
        if d_n.shape != c.shape:
            d_n = d_n.squeeze(-1) if d_n.ndim == 3 else d_n  # Ensure shape matches c
...
def state_trans(model,state_trans_pd,shocks,t=None):
    ...
    #print(f"House price t+1: {round(torch.mean(q_plus).item(), 5)}")
